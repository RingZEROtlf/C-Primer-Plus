...
char menu(void)
{
    ...
    {
        while (getchar() != '\n')   // deal with the left part of input line
            continue;
        ch = tolower(ch);
    ...
    if (ch == EOF)      // make this program exit
        ch = 'q';
    return ch;
}
void addpet(Tree *pt)
{
    ...
    else
    {
        ...
        s_gets(temp.petkinds[0], SLEN);
        temp.counter = 1;
        uppercase(temp.petname);
        uppercase(temp.petkinds[0]);
        AddItem(&temp, pt);
    }
}
void showpets(const Tree *pt)
...
void findpet(const Tree *pt)
{
    ...
    {
        puts("No entries!");
        return;                 // if tree is empty, exit this function
    }
    ...
    uppercase(temp.petname);
    uppercase(temp.petkinds[0]);
    printf("%s the %s ", temp.petname, temp.petkinds[0]);
    if (InTree(&temp, pt))
        printf("is a member.\n");
    else
